...
public final class Core {
    ...
    private void decodeQ0(int opCode) {
        switch (opCode) {
            ...
            case 0x01/*LD BC,nn*/ -> state.bc(bus.cpuReadMemWord(state.pcInc2())); // ZUM(102) HTP(293-294)
            ...
            case 0x11/*LD DE,nn*/ -> state.de(bus.cpuReadMemWord(state.pcInc2())); // ZUM(102) HTP(293-294)
            ...
            case 0x21/*LD HL,nn*/ -> state.hl(bus.cpuReadMemWord(state.pcInc2())); // ZUM(102) HTP(293-294)
            case 0x22/*LD (nn),HL*/ -> bus.cpuWriteMemWord(bus.cpuReadMemWord(state.pcInc2()), state.hl()); // ZUM(109) HTP(323-324)
            ...
            case 0x2A/*LD HL,(nn)*/ -> state.hl(bus.cpuReadMemWord(bus.cpuReadMemWord(state.pcInc2()))); // ZUM(105) HTP(334-335)
            ...
            case 0x31/*LD SP,nn*/ -> state.sp(bus.cpuReadMemWord(state.pcInc2())); // ZUM(102) HTP(293-294)
            case 0x32/*LD (nn),A*/ -> bus.cpuWriteMemByte(bus.cpuReadMemWord(state.pcInc2()), state.a()); // ZUM(97) HTP(319-320)
            case 0x33/*INC SP*/ -> state.sp(inc_nn(state.sp())); // ZUM(184) HTP(265-266)
            case 0x34/*INC (HL)*/ -> bus.cpuWriteMemByte(state.hl(), inc_n(bus.cpuReadMemByte(state.hl()))); // ZUM(161) HTP(267)
            case 0x35/*DEC (HL)*/ -> bus.cpuWriteMemByte(state.hl(), dec_n(bus.cpuReadMemByte(state.hl()))); // ZUM(164-165) HTP(238-239)
            case 0x36/*LD (HL),n*/ -> bus.cpuWriteMemByte(state.hl(), bus.cpuReadMemByte(state.pcInc1())); // ZUM(86) HTP(301-302)
            ...
            case 0x3A/*LD A,(nn)*/ -> state.a(bus.cpuReadMemByte(bus.cpuReadMemWord(state.pcInc2()))); // ZUM(94) HTP(317-318)
    ...
    private void decodeQ3(int opCode) {
        switch (opCode) {
            ...
            case 0xC5/*PUSH BC*/ -> bus.cpuWriteMemWord(state.spDec2(), state.bc()); // ZUM(116) HTP(379-380)
            ...
            case 0xCE/*ADC A,n*/ -> adc_a_n(bus.cpuReadMemByte(state.pcInc1())); // ZUM(146-147) HTP(190-191)
            ...
            case 0xD3/*OUT (n),A*/ -> bus.cpuWritePortByte((state.a() << 8) | bus.cpuReadMemByte(state.pcInc1()), state.a()); // ZUM(279) HTP(368)
            case 0xD4/*CALL NC,nn*/ -> call_t(!state.cf()); // ZUM(257-259) HTP(219-221)
            case 0xD5/*PUSH DE*/ -> bus.cpuWriteMemWord(state.spDec2(), state.de()); // ZUM(116) HTP(379-380)
            ...
            case 0xDB/*IN A,(n)*/ -> state.a(bus.cpuReadPortByte((state.a() << 8) | bus.cpuReadMemByte(state.pcInc1()))); // ZUM(269) HTP(263)
            case 0xDC/*CALL C,nn*/ -> call_t(state.cf()); // ZUM(257-259) HTP(219-221)
            case 0xDD/*[0xDD],...*/ -> decodeXY(0xDD, bus.cpuReadMemByte(state.pcInc1()), state::ix, state::ix);
            case 0xDE/*SBC A,n*/ -> sbc_a_n(bus.cpuReadMemByte(state.pcInc1())); // ZUM(150-151) HTP(420-421)
            ...
            case 0xE5/*PUSH HL*/ -> bus.cpuWriteMemWord(state.spDec2(), state.hl()); // ZUM(116) HTP(379-380)
            ...
            case 0xF5/*PUSH AF*/ -> bus.cpuWriteMemWord(state.spDec2(), state.af()); // ZUM(116) HTP(379-380)
            ...
            case 0xFD/*[0xFD],...*/ -> decodeXY(0xFD, bus.cpuReadMemByte(state.pcInc1()), state::iy, state::iy);
    ...
    private void decodeEDQ1(int opCode) {
        switch (opCode) {
            case 0x40/*IN B,(C)*/ -> state.b(bus.cpuReadPortByte(state.bc())); // ZUM(270-271) HTP(261-262)
            case 0x41/*OUT (C),B*/ -> bus.cpuWritePortByte(state.bc(), state.b()); // ZUM(280-281) HTP(366-367)
    ...
    private void decodeXYQ0(int prefix1, int opCode, IntSupplier rrGet, IntConsumer rrSet) {
        switch (opCode) {
            ...
            case 0x21/*LD IXY,nn*/ -> rrSet.accept(bus.cpuReadMemWord(state.pcInc2())); // ZUM(103) HTP(336-337)
            case 0x22/*LD (nn),IXY*/ -> bus.cpuWriteMemWord(bus.cpuReadMemWord(state.pcInc2()), rrGet.getAsInt()); // ZUM(111-112) HTP(325-328)
            ...
            case 0x26/*LD IXYH,n*/ -> high(rrGet, rrSet, bus.cpuReadMemByte(state.pcInc1())); // undocumented
            case 0x29/*ADD IXY,IXY*/ -> add_rr_nn(rrGet, rrSet, rrGet.getAsInt()); // ZUM(182-18) HTP(205-208)
            case 0x2A/*LD IXY,(nn)*/ -> rrSet.accept(bus.cpuReadMemWord(bus.cpuReadMemWord(state.pcInc2()))); // ZUM(107-108) HTP(338-339,342-343)
            ...
            case 0x2E/*LD IXYL,n*/ -> low(rrGet, rrSet, bus.cpuReadMemByte(state.pcInc1())); // undocumented
    ...
    private void decodeXYQ1(int prefix1, int opCode, IntSupplier rrGet, IntConsumer rrSet) {
        ...
        int val = switch (src) {
            ...
            case 0x06/*LD r,(IXY+n)*/ -> bus.cpuReadMemByte(indexed(rrGet.getAsInt())); // ZUM(84-85) HTP(305-308)
        ...
        switch (dst) {
            ...
            case 0x06/*LD (IXY+n),r*/ -> bus.cpuWriteMemByte(indexed(rrGet.getAsInt()), val); // ZUM(87-88) HTP(313-316)
    ...
    private void decodeXYQ3(int prefix1, int opCode, IntSupplier rrGet, IntConsumer rrSet) {
        switch (opCode) {
            ...
            case 0xDD/*[0xDD|0xFD],[0xDD],...*/ -> decodeXY(0xDD, bus.cpuReadMemByte(state.pcInc1()), state::ix, state::ix);
            case 0xE1/*POP IXY*/ -> rrSet.accept(bus.cpuReadMemWord(state.spInc2())); // ZUM(120) HTP(375-376)
            ...
            case 0xE5/*PUSH IXY*/ -> bus.cpuWriteMemWord(state.spDec2(), rrGet.getAsInt()); // ZUM(117) HTP(381-382)
            ...
            case 0xFD/*[0xDD|0xFD],[0xFD],...*/ -> decodeXY(0xFD, bus.cpuReadMemByte(state.pcInc1()), state::iy, state::iy);
            default -> nop_(prefix1, opCode);
        }
    }
    ...
    private void decodeXYCB(int address) {
        ...
        int qtr = opCode & 0xC0;
        switch (qtr) {
            case 0x00/*[0xDD|0xFD],[0xCB],[0x00-0x3F]*/ -> setAdditionalRegister(address, opCode, shf_rot_n(opCode, bus.cpuReadMemByte(address))); /* `RLC (IXY+n),r` ZUM(198-201) HTP(404-407), `RRC (IXY+n),r` ZUM(205-207) HTP(413-414), `RL (IXY+n),r` ZUM(202-204) HTP(396-397), `RR (IXY+n),r` ZUM(208-210) HTP(410-411), `SLA (IXY+n),r` ZUM(211-213) HTP(428-429), `SRA (IXY+n),r` ZUM(214-216) HTP(430-431), `SL1 (IXY+n),r` undocumented, SRL (IXY+n),r` ZUM(217-219) HTP(432-433) */
            case 0x40/*[0xDD|0xFD],[0xCB],[0x40-0x7F]*/ -> bit_m_n((opCode & 0x38) >>> 3, bus.cpuReadMemByte(address)); /* `BIT m,(IXY+n)` ZUM(228-231) HTP(213-216) */
            case 0x80/*[0xDD|0xFD],[0xCB],[0x80-0xBF]*/ -> setAdditionalRegister(address, opCode, res_m_n((opCode & 0x38) >>> 3, bus.cpuReadMemByte(address))); /* `RES m,(IXY+n),r` ZUM(236-237) HTP(385-387) */
            case 0xC0/*[0xDD|0xFD],[0xCB],[0xC0-0xFF]*/ -> setAdditionalRegister(address, opCode, set_m_n((opCode & 0x38) >>> 3, bus.cpuReadMemByte(address))); /* `SET m,(IXY+n),r` ZUM(234-235) HTP(425-427) */
        }
    }
