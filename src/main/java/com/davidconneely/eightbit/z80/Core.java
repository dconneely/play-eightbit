...
final class Core {
    ...
    private void decodeQ3(int opCode) {
        switch (opCode) {
            ...
            case 0xFD/*[0xFD],...*/ -> decodeXY(0xFD, bus.readMemory(state.pcInc1()), state::iy, state::iy);
    ...
    private void decodeXYQ3(int prefix1, int opCode, IntSupplier rrGet, IntConsumer rrSet) {
        switch (opCode) {
            ...
            case 0xDD/*[0xDD|0xFD],[0xDD],...*/ -> decodeXY(0xDD, bus.readMemory(state.pcInc1()), state::ix, state::ix);
            case 0xE1/*POP IXY*/ -> rrSet.accept(bus.readWord(state.spInc2())); // ZUM(120) HTP(375-376)
            case 0xED/*[0xDD|0xFD],[0xED],...*/ -> decodeED(bus.readMemory(state.pcInc1()));
            ...
            case 0xFD/*[0xDD|0xFD],[0xFD],...*/ -> decodeXY(0xFD, bus.readMemory(state.pcInc1()), state::iy, state::iy);
            default -> nop_(prefix1, opCode);
        }
    }
    ...
    private void setRegister(int r, int n) {
        switch (r & 0x07) {
            ...
            case 0x04 -> state.h(n);
            case 0x05 -> state.l(n);
            case 0x06 -> bus.writeMemory(state.hl(), n);
            case 0x07 -> state.a(n);
        }
    }
